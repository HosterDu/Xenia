...
class EventMutationTest {
    ...
    fun `verify createEvent mutation`() {
        ...
        val mutation = graphqlTestUtils.getMutation(Event::class.java, queryName = query,
                inputName = "event", input =newEvent.toCreateEventDto(),
                inputType = CreateEventDto::class.java)
    ...
    fun `verify createEvent mutation creates geolocation`() {
        ...
        val newEvent = EventFactory().`object`
        val mutation = graphqlTestUtils.getMutation(Event::class.java, queryName = query,
                subQueries  = listOf(Geolocation::class.java),
                inputName = "event", input =newEvent.toCreateEventDto(),
                inputType = CreateEventDto::class.java)
        testClient.post()
                .uri(GRAPHQL_ENDPOINT)
    ...
    fun `verify createEvent mutation adds creator`() {
        ...
        val newEvent = EventFactory().`object`
        val mutation = graphqlTestUtils.getMutation(Event::class.java, queryName = query,
                subQueries  = listOf(Profile::class.java),
                inputName = "event", input =newEvent.toCreateEventDto(),
                inputType = CreateEventDto::class.java)
